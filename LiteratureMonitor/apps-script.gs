...
const CANDIDATE_SHEET = 'CandidateSignals';
...
function doPost(e) {
  ...
  if (data.recordType === 'candidate') return handleCandidateSubmit_(data);
...
/* ---------------- Module 4: literature monitoring (candidate queue) ---------------- */

function handleCandidateSubmit_(data) {
  const sheet = getOrCreateSheet_(CANDIDATE_SHEET, SIGNAL_HEADERS);
  const linkCol = SIGNAL_HEADERS.indexOf('sourceLink');
  const existingLinks = sheet.getDataRange().getValues().slice(1).map(function (row) { return row[linkCol]; });

  if (data.sourceLink && existingLinks.indexOf(data.sourceLink) !== -1) {
