...
function doGet(e) {
  ...
  if (action === 'aggregates') return jsonOutput_(getAggregates_());
...
function getAggregates_() {
  ...
  values.slice(1).forEach(function (row) {
    if (String(row[idx.round]) !== '1') return;
  ...
  Object.keys(groups).forEach(function (sid) {
    ...
    result[sid] = {
      ...
      plausibility: TIER_LABELS[median_(g.plausibility)],
      significance: TIER_LABELS[median_(g.significance)],
      urgency: TIER_LABELS[median_(g.urgency)]
    };
  });
  return result;
}
